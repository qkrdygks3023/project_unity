...
public class dataController : MonoBehaviour
{
    ...
    public void JsonSave()
    {
        ...
        // saveData.Data3.Add(new Datajson("구술영상", 2022, "안태진 육군 포병", "1. 국군/경찰", "2022년", new List<string> { "구술영상", "인터뷰", "구술채록", "구술사", "참전자", "참전용사", "6·25전쟁", "한국전쟁", "육군", "수도기계화보병사단", "수도사단", "금성전투", "713전투", "351고지전투", "육근수" }, 521, "전쟁의 참상을 표현한다 다큐멘터리", "/movies/03/9.안태진(2022).mp4", "/images/03/ing1.png"));
    ...
    public void JsonLoad()
    {
        ...
        List<contentData> sorted = new List<contentData>();
        if (select_type == 3)
        {
            sorted = contentData.OrderBy(ob => ob.year).ToList();
        }
        else
        {
            sorted = contentData.OrderBy(ob => ob.title).ToList();
        }
        ...
        {
            //군국
            string selectmainString = select_main.Split('.')[1].ToString();
            selectController.yeartitleObj.GetComponentInChildren<TextMeshProUGUI>().text = selectmainString;
        }
        else
        {
            Debug.Log(" select_main      select_mainselect_mainselect_mainselect_mainselect_main **" + select_main);
            //연도별 타이틀
            if (select_type == 0)
            {
            ...
            {
                //1950년대
                selectController.yeartitleObj.GetComponentInChildren<TextMeshProUGUI>().text = "연대별";
            }
...
public class Datajson01
{
    ...
    public Datajson01(string _type, string _group_year, string _title, string _place, string _year, string _keyword, int _playtime, string _summary, string _url, string _thumbnail)
    {
        type = _type;
        group_year = _group_year;
        title = _title;
